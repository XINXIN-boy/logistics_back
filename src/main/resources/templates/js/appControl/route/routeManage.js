    ...
    table.on('tool(expandTool)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('真的删除么', function(index){
                ...
                $.ajax({
                    type: "get",
                    url: "/route/deleteRegions.do?id=" + data.id,
                    dataType: 'json',
                    async: false,
                    success: function (result) {
                        console.log(result);
                ...
                table.reload('expandTable', {
                    url: '/route/findAllExpands.do?page=1&limit=10'
                })
            });
...
//这里 findLeftRegions
    ...
    function refreshTable() {
        table.render({
            elem: '#expandTable',
            height: 'full-170',
            url:'/route/findAllExpands.do', //数据接口
            limit: 10,
            limits: [ 10 ],
            request: {
