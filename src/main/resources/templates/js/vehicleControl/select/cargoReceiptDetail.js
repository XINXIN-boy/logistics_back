layui.use(['element', 'form', 'laydate', 'layer', 'table'], function () {
    ...
    table.on('tool(cargoReceiptTool)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('真的删除么', function(index){
                ...
                $.ajax({
                    type: "get",
                    url: "/vehicle/deleteCargoreceipt.do?goodsRevertBillCode=" + data.goodsRevertBillCode,
            ...
            layer.open({
                ...
                move: false,
                    ...
                    table.reload('cargoReceiptTable1', {
                        url: '/vehicle/findAllCargoreceiptByEvent.do?page=1&limit=10&backBillState=' + array[0]
                    })
                }
            });
    ...
    function refresh(id) {
        table.render({
            ...
            height: 'full-170',
            url:'/vehicle/findAllCargoreceiptByEvent.do?backBillState=' + (id !== 3 ? array[id] : ''),
            limit: 10,
            limits: [10],
            request: {
